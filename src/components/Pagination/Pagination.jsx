...
export const Pagination = ({ pagination }) => {
  const { currentPage, limit, totalPages, totalProducts } = pagination;
  ...
  const paginationCurrent =
    currentPage === totalPages ? totalProducts : currentPage * limit;
  ...
  return (
    <div className={s.pagination}>
      <div
        className={s.bar}
        style={{
          "--width": `calc(${paginationCurrent}/${totalProducts} * 100%)`,
        }}></div>
      <div className={s.arrays}>
        ...
        <p className={s.info}>
          <span>{paginationCurrent}</span> из <span>{totalProducts}</span>
        </p>
